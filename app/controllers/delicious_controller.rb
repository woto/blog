class DeliciousController < ApplicationController
  ...
  def index
     ...
     l.each{|i|
       ...
       p.intro = "<p><a href='#{i.url.to_s}'><img alt='#{i.url.to_s}' src='http://images.websnapr.com/?size=size&key=YL9563thKaIT&url=#{URI::encode(i.url.to_s)}'></img></a></p><p>#{i.notes.to_s}</p><p><a href='#{i.url.to_s}'>#{i.url.to_s}</a></p>"
       p.date = i.time
       p.body = p.intro
       p.tag_list = i.tags
