  ...
  def index
   feed = Feedzirra::Feed.fetch_and_parse(AppConfig.google_reader)
    feed.entries.each do |entry|
      ...
      p.body = p.body.to_s + "<br /><p><a href='#{entry.url.to_s}'><img alt='#{entry.url.to_s}' src='http://images.websnapr.com/?size=size&key=YL9563thKaIT&url=#{URI::encode(entry.url.to_s)}'></img></a></p><p><a href='#{entry.url.to_s}'>#{entry.url.to_s}</a></p>"
      p.intro = p.body
